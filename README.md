...
Practice attempts are idempotent, can be queued offline, and return `correct`, four beats, and a `ClientView` (`bandLabel`, `showConceptChip`, `celebrationTier` of `none`, `quietXp`, or `full`). The practice screen leads with Correct or Not yet from that `correct` field. A correct try labels the beats Nice move, Why it works, Try next, and Answer. Why it works is stored only when the item bank has a solidify line, and an empty beat is left out. A miss labels them What you tried, One focus, Try next, and Lock in. There is no score and no confidence value.
...
Slice 7 keeps kid-visible fuel on that same bus, and only the rules estimator mints it. The flame is a replay of `QualifyingPracticeDay` rows, so a streak column cannot heat on its own. XP stays an accrue-only credit pointed at a qualifying event: time on the app, a soft score, and a try that does not mint an event do not add any, and nothing subtracts it. Build pieces stay the Slice 5 projection. The child home shows those three as tinted chips under the single Start practice button: a flame mark and `{n}-day flame`, a star and `{xp} XP`, and a piece mark and `{pieces}/{goal}`. Before the first qualifying day the flame chip says `Start your flame` and the others show 0 XP and 0 pieces. A cooled or dormant flame says `Flame resting` and does not show a zero day count. The strip is not a link. A qualifying mint pulses that strip once when the child returns home, the same moment practice would show the mint toast. Practice itself stays the destination: a saved try may show one quiet sprout line, and a full try may add one piece beat. Review shows the quiet line only. The parent one-breath card stays minutes, focus, and band, with no XP.

The companion payload includes a derived read-only `glance`. `glance.xp` is the sum of XP credits that point at QualifyingEvents. `glance.dayCount` is the current consecutive qualifying-day run in `child.timezone`, or null before the first qualifying day and while the flame is cooled or dormant. `glance.pieces` and `glance.goal` are the filled pieces and the target of the one active BuildGoal. That glance does not mint, and it is not a second ledger. The payload still has no score percent, confidence, or judgment copy.

This derived total supersedes the earlier line that the companion carried no XP total and the home strip had no count (Opportunity memo §11d amendment, Architecture §25). Each number is a server-side read of rows that already exist: the flame from the `QualifyingPracticeDay` run, XP from ledger credits that point at QualifyingEvents, and pieces from the one BuildGoal projection. There is no counter column or table a request can increment, and there is no client write path. Practice keeps the quiet sprout line and does not show a running XP count. The total never decreases, and the copy does not compare, rank, or keep a leaderboard. The counts are fuel state only. They do not feed Learner State, `MasteryEstimator`, or band decisions. The parent one-breath card stays XP-free.
...
New attempts and practice sessions also store `build_sha` beside `policy_version`. `APP_BUILD_SHA` is required for any deploy that is not a git checkout. When that variable is set, the server uses that value and does no git work: no prime, no stamp reads, and no refresh. Without it, startup starts one background refresh of `git describe --always --dirty --abbrev=12`. Later saves read the cache. A changed `.git/HEAD` or about 5 seconds starts another refresh, and only one git process runs at a time. A save never waits on git. A save made within about 5 seconds of a git pull may record the previous SHA, and that lag is accepted. A refresh that times out or fails keeps the last known tag, or `unknown` when there has never been one. Startup logs one warning when the tag ends up `unknown`, and before/after analysis excludes those rows. The process does not crash. A new row never stores an empty tag. Rows from before the column stay null. The server attempt log line includes the tag next to `policy_version`. The child client does not receive it. The parent home shows a muted `Build <short sha>` footer. Child routes do not.
...
Practice problems are issued on the server from versioned templates. Each of the 11 skills has the original item as a legacy `v0` seed plus at least three generated shapes. Progression stays `rules-v0`. Each skill's assigned step is 1 (Warm-up), and issuance does not change that assignment. When the step-1 pool is exhausted, issuance switches to another skill at step 1, and only then repeats the oldest item of the requested skill at that step. It does not draw a harder step. Steps 2 and 3 stay on the template for later policies. The server copies `template_version`, `evidence_eligible`, and the canonical answer onto `item_instances` at issue time. A repeat is a new instance id with `evidence_eligible` set false at issue, so an attempt on it is not evidence. Scoring reads that copy. A blank answer is still review-lane evidence and mints nothing. An unreadable answer is not an attempt: the server returns `format_rejected`, mints nothing, and logs the reject without the child's text. The issued child payload carries `answerKind` from the template family, even when a fraction answer simplifies to a whole number, and a server-chosen `formatExample` that is not the answer. Lock versus retry is the one constant `UNPARSEABLE_BEHAVIOR`, currently `retry`. See `docs/item-templates.md`.
...
3. Grant, pause, or revoke consent from the parent home. Only `granted` allows practice. The parent home carries the consent line: only a parent can allow practice, and missing, paused, or revoked consent does not start a session. Each child card also shows today's one-breath story: minutes, the focus concept, and the band. Before any committed practice, that story is "No practice yet."
4. Open the child home. The greeting sits above today's focus and band. Start practice is the only practice control. Pause, revoke, and missing consent replace that button with a calm note and leave the fuel strip in place. The offline cap stays a sync limit and does not say practice is paused.
5. With consent granted, start practice. Answer a problem from the operations or fractions pack. Correct or Not yet sits under the problem before the beats. A correct try reads Nice move, Try next, and Answer, and adds Why it works only when the item bank has a solidify line. A miss reads What you tried, One focus, Try next, and Lock in. One focus is the server string from that item's misconception tag, shown as sent, and an empty string is left out. Answer and Lock in show the value, such as `27 + 15 = 42`. A queued try does not move the skill band until the server commits it. The response does not show a score or a confidence number.
6. If the connection drops while consent is still granted, the answer stays in a device queue and syncs with the same idempotency key when the connection returns. A replay returns the original attempt, the original `ClientView`, and the original event ids. That response is the saved try, not an empty 409. The unsynced queue holds at most 3 tries. At that cap the child stays on the same problem, and the parent home shows a sync-limit note. That note does not say practice is paused. Pause holds a pending queue for a parent-visible wait and a quiet resume. Pause does not drop the queue. Revoke drops the queue and does not sync it. This offline queue is not claimed as a kid-reachable ship. On a miss, one focus is the server string assembled from that item's misconception tag, shown as sent.
...
8. On the child home, Start practice is the only button. Under it, the fuel strip shows the qualifying-day count, accrued XP, and filled pieces over the active goal. Badges and the pot, including open slots, are behind the Badges link. During practice, a saved try shows one quiet line when a mint happened, and a full try may add one piece beat. A blank try says there is no sprout. A review try stays on that quiet line. Returning home after that mint pulses the strip once.
...
| GET | `/api/children/:id/companion` | The child companion: one active BuildGoal, badge rows, the streak surface, and a derived read-only `glance`. `glance.xp` sums XP credits that point at QualifyingEvents. `glance.dayCount` is the consecutive qualifying-day run in `child.timezone`, or null before the first qualifying day and while the flame is cooled or dormant. `glance.pieces` and `glance.goal` are the filled pieces and the target of the one active BuildGoal. Ember includes a recovery copy key. This payload has no score percent, confidence, or judgment copy. This derived total supersedes the earlier rule of no XP total and no count (Opportunity memo §11d amendment, Architecture §25). The numbers are a server-side read, not a counter, they do not tick per attempt, they never decrease, and they do not feed Learner State, `MasteryEstimator`, or band decisions. The parent one-breath payload stays XP-free. |
...
- a correct try stores a bank solidify line in `oneFocus` when one exists, and a miss stores the misconception tag, rendered as sent
...
- new attempts and sessions store a non-empty `build_sha` beside `policy_version`, the submit path logs that line, older rows may leave it null, child payloads omit `build_sha` and `policy_version`, and the parent home footer shows the short sha
...
- Ember is the only streak state with a recovery copy key, and Warm, Hot, and Dormant are not; the child home does not render that key as a button
- the child home and parent home payloads stay free of a second progress ledger
- the companion `glance` is a derived read of XP, day count, and active-goal pieces, and it does not mint
- that glance supersedes the earlier "no XP total / no count" line (Opportunity memo §11d amendment, Architecture §25): it is a server-side read, not a counter, it does not tick on the practice screen, it never decreases, and it does not feed Learner State, `MasteryEstimator`, or band decisions
...
- the child home has one practice control, with the numeric fuel strip under it, and the parent one-breath card still has no XP
- a cooled or dormant flame reads Flame resting, and the strip before the first qualifying day stays visible with zero XP and zero pieces
- pause replaces the practice control with calm copy and leaves the strip; offline-cap copy never says paused
- volume that does not mint a qualifying event leaves the strip numbers unchanged
