...
Slice 7 keeps kid-visible fuel on that same bus, and only the rules estimator mints it. The flame on the child home is a replay of `QualifyingPracticeDay` rows, so a streak column cannot heat on its own. XP stays an accrue-only credit pointed at a qualifying event: time on the app, a soft score, and a try that does not mint an event do not add any, and nothing subtracts it. Build pieces stay the Slice 5 projection. The home shows those three in a strip under the Practice button. Practice itself stays the destination: a saved try may show one quiet sprout line, and a full try may add one piece beat. Review shows the quiet line only. Ember copy invites a careful try without scolding a cool-down. A live companion reaction inside practice is deferred so it does not become a second destination. The parent one-breath card stays minutes, focus, and band.
...
8. On the child home, Start practice is the button. Under it, a strip shows the flame, the sprout, and a piece from the qualifying bus, without a count and without a second screen. The sprout card below still shows the flame and the one active build. Open spots fill when the bus mints a badge, a slightly harder step, or a hot-streak piece. Badges are listed on their own screen. If the flame is an ember, the copy invites a careful try and does not scold the cool-down. During practice, a saved try shows one quiet line when a mint happened, and a full try may add one piece beat. A blank try says there is no sprout. A review try stays on that quiet line.
