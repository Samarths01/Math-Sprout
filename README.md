...
New attempts and practice sessions also store `build_sha` beside `policy_version`. `APP_BUILD_SHA` is required for any deploy that is not a git checkout. When that variable is set, the server reads it once at startup and does not spawn git. Without it, startup starts one background refresh of `git describe --always --dirty --abbrev=12`. Later saves read the cache. A changed `.git/HEAD` or about 5 seconds starts another refresh, and only one git process runs at a time. A save never waits on git. A refresh that times out or fails keeps the last known tag, or `unknown` when there has never been one. Startup logs one warning when the tag ends up `unknown`, and before/after analysis excludes those rows. The process does not crash. A new row never stores an empty tag. Rows from before the column stay null. The server attempt log line includes the tag next to `policy_version`. The child client does not receive it. The parent home shows a muted `Build <short sha>` footer. Child routes do not.
