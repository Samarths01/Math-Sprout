...
New attempts and practice sessions also store `build_sha` beside `policy_version`. `APP_BUILD_SHA` is required for any deploy that is not a git checkout. When that variable is set, the server reads it once at startup. Without it, each new attempt and session resolves `git describe --always --dirty --abbrev=12` (cached until `.git/HEAD` changes, and refreshed about every 5 seconds so a dirty tree is picked up). If neither resolves, the tag is the literal `unknown`, startup logs one warning, and before/after analysis excludes those rows. The process does not crash. A new row never stores an empty tag. Rows from before the column stay null. The server attempt log line includes the tag next to `policy_version`. The child client does not receive it. The parent home shows a muted `Build <short sha>` footer. Child routes do not.
