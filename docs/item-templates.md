...
When an item is issued, the child payload carries `answerKind` (`whole` or `fraction`) and `formatExample`. The server picks the example and it is never equal to the canonical answer: whole numbers use `3`, or `4` when the answer is 3; fractions use `1/2`, or `1/3` when the answer is equivalent to 1/2 (including `2/4`). The client does not compute or receive the answer. Offline-batch items carry the same two fields. The hint copy lives in `FORMAT_HINT_COPY`: `Use numbers only, like {example}.` and `Write it as a fraction, like {example}.`
